#!/bin/bash
# Old Prompt Clarity Conditioning Simulation Runner
...
run_model() {
    ...
    echo ""
    echo "=================================================="
    echo "[${EASY_NAME}] Starting Old Prompt Conditioning Batch (Model ID: ${MODEL})"
    echo "=================================================="

    # Create output directories
    mkdir -p "final_results/data/conditioning/goal_clarity/adv_pressure/old_prompt_conditioning/${EASY_NAME}/"
    mkdir -p "final_results/logs/conditioning/goal_clarity/adv_pressure/old_prompt_conditioning/${EASY_NAME}/"
    
    # Create model-specific temp checkpoint directory to avoid race condition
    TEMP_CHECKPOINT_DIR="temp_checkpoints_old_prompt/${EASY_NAME}/"
    mkdir -p "${TEMP_CHECKPOINT_DIR}"

    for run in "${RUNS_ARRAY[@]}"; do
        echo "[${EASY_NAME}] Running weaker prompt conditioning simulation (run ${run})..."
        ...
        dest_pkl="final_results/data/conditioning/goal_clarity/adv_pressure/old_prompt_conditioning/${EASY_NAME}/checkpoint_run${run}_41.pkl"
        if [ -f "${src_pkl}" ]; then
            mv "${src_pkl}" "${dest_pkl}"
        ...
        dest_file="final_results/logs/conditioning/goal_clarity/adv_pressure/old_prompt_conditioning/${EASY_NAME}/task1_output_run${run}_41.txt"
        if [ -f "${src_file}" ]; then
...
echo ""
echo "========================================"
echo "Starting Old Prompt Conditioning Simulations"
...
echo ""
echo "========================================"
echo "All old prompt conditioning simulations completed!"
echo "========================================"
