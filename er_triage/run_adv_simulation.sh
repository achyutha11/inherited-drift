#!/bin/bash
# Adversarial Pressure Simulation Runner
...
for model_entry in "${MODELS[@]}"; do
    ...
    echo ""
    echo "========================================"
    echo "Starting ${EASY_NAME} simulations"
    echo "========================================"
    
    # Create output directories if they don't exist
    mkdir -p "final_results/data/basic/adv_pressure/${EASY_NAME}"
    mkdir -p "final_results/logs/basic/adv_pressure/${EASY_NAME}"
    
    for run in "${RUNS_ARRAY[@]}"; do
        echo ""
        echo "Running ${EASY_NAME} adversarial pressure simulation (run ${run})..."
        ...
        dest_json="final_results/data/basic/adv_pressure/${EASY_NAME}/checkpoint_run${run}_$((NUM_STEPS + 1)).json"
        if [ -f "${src_json}" ]; then
        ...
        dest_pkl="final_results/data/basic/adv_pressure/${EASY_NAME}/checkpoint_run${run}_${NUM_STEPS}.pkl"
        if [ -f "${src_pkl}" ]; then
            mv "${src_pkl}" "${dest_pkl}"
            echo "✓ Saved checkpoint to ${dest_pkl}"
        fi
        
        # Move trajectory log (ER-goal-drift uses MODEL for log directory, not EASY_NAME)
        src_file="logs_${MODEL}/run_${run}_trajectory.txt"
        dest_file="final_results/logs/basic/adv_pressure/${EASY_NAME}/run_${run}_trajectory.txt"
        if [ -f "${src_file}" ]; then
