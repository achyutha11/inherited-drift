...
def setup_logging(run, model, verbose, log_name=None):
    # Use log_name if provided, otherwise sanitize model name (replace slashes and colons)
    if log_name:
        safe_name = log_name
    else:
        safe_name = model.replace('/', '_').replace(':', '_')
    
    log_file_path = f'logs_{safe_name}/task1_output_run{run}.txt'

    os.makedirs(f'logs_{safe_name}', exist_ok=True)
...
def run_single_simulation(run, model, results_file, lock, verbose, use_soft_system_prompt, sim_params):
    try:
        ...
        skip_prompt_caching = sim_params.pop('skip_prompt_caching', False)
        setup_logging(run, model, verbose, log_name=log_name)
        ...
        move_sys_goal = sim_params['move_sys_goal']
        sim_manager = SimulationManager(model, run, branched_from, remove_goals, reverse_goal, move_sys_goal, 
                                         extended_thinking=sim_params.get('extended_thinking', False),
                                         thinking_budget=sim_params.get('thinking_budget', 10000),
                                         provider_order=provider_order,
                                         skip_prompt_caching=skip_prompt_caching)
...
def parse_args():
    ...
    parser.add_argument("--move_sys_goal", action="store_true", help="Move the system goal to the first user message.")
    ...
    parser.add_argument("--extended_thinking", action='store_true', help="Enable extended thinking for Claude Sonnet 4.5+ models.")
    parser.add_argument("--thinking_budget", type=int, default=10000, help="Budget tokens for extended thinking (default: 10000).")
    parser.add_argument("--log_name", type=str, help="Custom name for log directory (useful for models with slashes/colons in name).")
    parser.add_argument("--provider_order", type=str, nargs='+', help="OpenRouter provider order (e.g., AtlasCloud NovitaAI Together).")
    parser.add_argument("--skip_prompt_caching", action='store_true', help="Disable prompt caching for Claude models (useful for conditioning runs).")
...
if __name__ == "__main__":
    ...
    sim_params = {
        ...
        'move_sys_goal': args.move_sys_goal,
        ...
        'skip_prompt_caching': args.skip_prompt_caching
    }
